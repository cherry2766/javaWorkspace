...
public class StudentDAO implements IStudentDAO {
	...
	public void insertStudent(StudentDTO dto) {
		...
		try {
			...
			// pstmt.setDate(5, dto.getStdBirth()); //dto.getStdBirth() util.date 반환,
			// pstmt.setDate()는 sql.date 요구
	...
	public StudentDTO detailStudent(String stdNo) {
		...
		try {
			...
			// 받은 결과를 dto에 추가해서 반환(기본키 기준으로 검색 진행 = 반환 결과 레코느는 1이거나 1)
			if (rs.next()) {
				stdNo = rs.getString(1); // 매개변수 전달받은 값이나 rs 1번 컬럼 값이나 동일해야 함
				String stdName = rs.getString(2);
				int stdYear = rs.getInt(3);
				String stdAddress = rs.getString(4);
				Date stdBirthday = rs.getDate(5);
	...
	public void updateStudent(StudentDTO dto) {
		...
		try {
			...
			if (result > 0) {
				...
				System.out.println("학번 " + dto.getStdNo() + " 학생의 정보가 없습니다");
			}
	...
	public ArrayList<StudentDTO> searchStudent(String dptNo) {
		...
		try {
			...
			if (rs.next()) {
				do {
					std = new StudentDTO(rs.getString(1), rs.getString(2), rs.getInt(3), rs.getString(4), rs.getDate(5),
							rs.getString(6));
					stdList.add(std);
				} while (rs.next());
			} else {
